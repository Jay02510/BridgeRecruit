...
Updated after the client-feedback QoL pass (import overhaul, institution profiles, reports, calendar).
...
- [ ] **Have Julian's real `.xlsx` on hand** and run it through Import Spreadsheet live — this is the strongest demo moment now that it's a named, exact-column path for his file specifically. Confirm the "Recognized this as your Partner Interactions sheet" card appears, and check the unrecognized-columns line (should read "every column is recognized" for his real file).
...
2. Import his real spreadsheet → jump to a school's profile, show real health status (not stalled) from imported meeting history
...
- [x] Legacy spreadsheet import rebuilt: named exact path for Julian's sheet (creates real interactions/follow-ups, not notes), fuzzy fallback for anything else, unrecognized-column warning, single unified "Import Spreadsheet" button (old strict CSV importer removed).
